...
{
    _hw_pin.parent.type         = RT_Device_Class_Miscellaneous;  //杂类设备
    _hw_pin.parent.rx_indicate  = RT_NULL;						  //设备回调
    _hw_pin.parent.tx_complete  = RT_NULL;						  //设备回调

#ifdef RT_USING_DEVICE_OPS							//使用ops占用ram更少，健壮性更好。
    _hw_pin.parent.ops          = &pin_ops;			
#else
    _hw_pin.parent.init         = RT_NULL;			 //通用的设备接口
    ...
    _hw_pin.ops                 = ops;				//GPIO设备操作结构体
    _hw_pin.parent.user_data    = user_data;		//设备私用数据

    /* register a character device */
    rt_device_register(&_hw_pin.parent, name, RT_DEVICE_FLAG_RDWR);		//注册设备
